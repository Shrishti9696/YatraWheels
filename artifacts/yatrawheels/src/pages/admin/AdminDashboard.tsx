...
import {
  ...
  LayoutDashboard, BookOpen, Truck, UserCog, Shield, CheckCircle2,
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const monthlyBookings: any[] = data?.monthlyBookings ?? [];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-card border border-card-border rounded-2xl overflow-hidden">
              <div className="px-6 py-4 border-b border-white/8">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Total collected vs. platform earnings</p>
              </div>
              <div className="p-4">
                {loading ? (
                    ...
                    <AreaChart data={monthlyRevenue} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      <defs>
                        <linearGradient id="gradRevenue" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="#5b67f5" stopOpacity={0.3} />
                          <stop offset="95%" stopColor="#5b67f5" stopOpacity={0} />
                        </linearGradient>
                      ...
                      <YAxis tickFormatter={formatINR} tick={{ fontSize: 11, fill: "#666" }} axisLine={false} tickLine={false} width={50} />
                      <Tooltip contentStyle={tooltipStyle} formatter={(v: any) => [`₹${Number(v).toLocaleString()}`, ""]} />
                      <Legend wrapperStyle={{ fontSize: 11, paddingTop: 8 }} />
                      <Area type="monotone" dataKey="revenue" name="Total Revenue" stroke="#5b67f5" fill="url(#gradRevenue)" strokeWidth={2} dot={false} />
                      <Area type="monotone" dataKey="platformFee" name="Platform Fee" stroke="#10b981" fill="url(#gradPlatform)" strokeWidth={2} dot={false} />
                    </AreaChart>
                  </ResponsiveContainer>
                )}
              </div>
            ...
            <div className="bg-card border border-card-border rounded-2xl overflow-hidden">
              <div className="px-6 py-4 border-b border-white/8">
                <h2 className="font-semibold">Bookings (Last 6 Months)</h2>
                <p className="text-xs text-muted-foreground mt-0.5">Number of bookings per month</p>
              </div>
              <div className="p-4">
                {loading ? (
                  <Skeleton className="h-52 w-full rounded-xl" />
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={monthlyBookings} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                      <XAxis dataKey="month" tick={{ fontSize: 11, fill: "#666" }} axisLine={false} tickLine={false} />
                      <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: "#666" }} axisLine={false} tickLine={false} width={30} />
                      <Tooltip contentStyle={tooltipStyle} />
                      <Bar dataKey="bookings" name="Bookings" fill="#5b67f5" radius={[6, 6, 0, 0]} maxBarSize={40} />
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-card border border-card-border rounded-2xl overflow-hidden">
              ...
              <div className="divide-y divide-white/5">
                {loading
                  ? Array.from({ length: 4 }).map((_, i) => <div key={i} className="px-6 py-4"><Skeleton className="h-10 w-full" /></div>)
                      ...
                      <div key={b._id} className="px-6 py-4 flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-muted-foreground truncate">
                            {b.vehicleId?.name || "Vehicle"} · {b.withDriver ? "With driver" : "Self-drive"}
                          </div>
                        </div>
                        <div className="text-right shrink-0">
                          <div className="text-sm font-bold text-emerald-400">₹{b.totalPrice?.toLocaleString()}</div>
                          <div className="text-xs text-muted-foreground">paid</div>
                        </div>
                      </div>
                    ))
                    : <div className="px-6 py-8 text-center text-muted-foreground text-sm">No paid bookings yet.</div>
                }
              </div>
            </div>
          </div>
