...
import {
  ...
  BookOpen, Route, Heart, TrendingUp, Star, ArrowLeft
} from "lucide-react";
...
import { mockVehicles, type Booking } from "@/data/mockData";
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {bookings.map((booking, i) => {
                    const vehicle = mockVehicles.find(v => v.id === booking.vehicleId) || mockVehicles[0];
                    return (
                      <motion.div
                        key={booking.id}
                      ...
                      >
                        <div className="w-20 h-16 rounded-xl overflow-hidden shrink-0">
                          <img src={vehicle.imageUrl} alt={vehicle.name} className="w-full h-full object-cover" loading="lazy" />
                        </div>
