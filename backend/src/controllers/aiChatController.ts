...
Make the plan detailed, exciting, and specific to the destination. Include real places.
...
export async function handleAIChat(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    let fallback: string;
    if (wantsYes && dest) {
      fallback = `I'd love to create your ${dest} plan! Our AI is temporarily at capacity — please try again in a moment, or browse our available fleet while you wait. Your trip details have been saved. 🙏`;
